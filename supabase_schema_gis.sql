...
-- ============================================
-- TABLA: usuarios (perfil de usuario)
-- ============================================
...
CREATE INDEX idx_trabajadores_user_id ON trabajadores(user_id);
...
-- POLÍTICAS: fincas
-- ============================================
CREATE POLICY "Los usuarios pueden ver sus propias fincas"
    ON fincas FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden crear sus propias fincas"
    ON fincas FOR INSERT
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden actualizar sus propias fincas"
    ON fincas FOR UPDATE
    USING (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden eliminar sus propias fincas"
...
-- POLÍTICAS: cuarteles
-- ============================================
CREATE POLICY "Los usuarios pueden ver sus propios cuarteles"
    ON cuarteles FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden crear sus propios cuarteles"
    ON cuarteles FOR INSERT
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden actualizar sus propios cuarteles"
    ON cuarteles FOR UPDATE
    USING (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden eliminar sus propios cuarteles"
...
-- ============================================
CREATE POLICY "Los usuarios pueden ver sus propios trabajadores"
    ON trabajadores FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden crear sus propios trabajadores"
    ON trabajadores FOR INSERT
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden actualizar sus propios trabajadores"
    ON trabajadores FOR UPDATE
    USING (auth.uid() = user_id);

CREATE POLICY "Los usuarios pueden eliminar sus propios trabajadores"
    ON trabajadores FOR DELETE
    USING (auth.uid() = user_id);

-- ============================================
-- POLÍTICAS: tipos_labor
-- ============================================
-- Los tipos de labor son compartidos entre todos los usuarios
CREATE POLICY "Todos pueden ver tipos de labor"
    ON tipos_labor FOR SELECT
    USING (true);

-- ============================================
-- POLÍTICAS: registros_horas
-- ============================================
CREATE POLICY "Los usuarios pueden ver sus propios registros"
    ON registros_horas FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM trabajadores t
            WHERE t.id = registros_horas.trabajador_id
            AND t.user_id = auth.uid()
        )
    );

CREATE POLICY "Los usuarios pueden crear sus propios registros"
    ON registros_horas FOR INSERT
    WITH CHECK (
        EXISTS (
            SELECT 1 FROM trabajadores t
            WHERE t.id = registros_horas.trabajador_id
...
-- ============================================
-- INSERTAR TIPOS DE LABOR POR DEFECTO
-- ============================================
INSERT INTO tipos_labor (nombre, descripcion) VALUES
    ...
    ('Desmalezado', 'Control de malezas'),
    ('Plantación', 'Plantación de cultivos'),
...
-- ============================================
COMMENT ON TABLE usuarios IS 'Perfiles de usuarios registrados';
